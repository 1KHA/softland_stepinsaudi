    ...
    <div className="min-h-screen bg-brand-bg" dir={dir}>
      ...
      <div className="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className={`${isArabic ? 'border-r border-gray-200 pr-4' : 'border-l border-gray-200 pl-4'}`}>
            <h1 className="text-xl font-bold text-[#2B3E8F]">
        ...
        <div className="hidden md:flex items-center gap-8">
          <button onClick={() => navigate('/employee-dashboard')} className="text-[#2B3E8F] font-semibold border-b-2 border-[#1DBAEA] pb-0.5 text-sm">
            {t('employee.nav.home')}
          </button>
          <button onClick={() => navigate('/employee-requests')} className="text-[#2B3E8F]/70 hover:text-[#1DBAEA] font-medium text-sm transition">
            {t('employee.nav.requests')}
          </button>
          <button onClick={() => navigate('/employee-documents')} className="text-[#2B3E8F]/70 hover:text-[#1DBAEA] font-medium text-sm transition">
            {t('employee.nav.documents')}
          </button>
          <button onClick={() => navigate('/employee-notifications')} className="relative text-[#2B3E8F]/70 hover:text-[#1DBAEA] font-medium text-sm transition flex items-center gap-1.5">
      ...
      <div className="p-8">
        ...
        <div className="mb-6 bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold text-[#2B3E8F] mb-4">{t('employee.quickActions')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { icon: <FileText className="w-5 h-5 text-[#1DBAEA]" />, label: t('employee.nav.requests'), to: '/employee-requests' },
              { icon: <Download className="w-5 h-5 text-[#1DBAEA]" />, label: t('employee.nav.documents'), to: '/employee-documents' },
              { icon: <Bell className="w-5 h-5 text-[#1DBAEA]" />, label: t('employee.nav.notifications'), to: '/employee-notifications' },
            ].map((item, i) => (
              <button key={i} onClick={() => navigate(item.to)}
                className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:border-[#1DBAEA] hover:bg-[#1DBAEA]/5 transition text-right">
                {item.icon}
                <span className="font-medium text-[#2B3E8F] text-sm">{item.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-48 bg-white rounded-2xl border border-gray-100">
            <div className="w-10 h-10 border-4 border-[#1DBAEA] border-t-transparent rounded-full animate-spin" />
          </div>
        ) : (
          <>
            {/* Stats */}
                ...
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-[#2B3E8F] mt-1">{card.value}</p>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-xl font-bold text-[#2B3E8F]">{t('employee.recentRequests')}</h2>
                  <button onClick={() => navigate('/employee-requests')}
                    className="text-[#1DBAEA] font-medium text-sm flex items-center gap-1 hover:gap-2 transition-all">
                      ...
                      <tbody>
                        {recentRequests.map(req => (
                          <tr key={req.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                            <td className="py-4 px-4 text-sm font-mono font-medium text-[#2B3E8F]">
                            ...
                            <td className="py-4 px-4">
                              <button onClick={() => navigate(`/employee-requests/${req.id}`)}
                                className="flex items-center gap-1.5 text-[#1DBAEA] font-medium text-sm hover:text-[#2B3E8F] transition">
              ...
              <div className="space-y-5">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold text-[#2B3E8F]">{t('employee.recentNotifications')}</h2>
                    <button onClick={() => navigate('/employee-notifications')} className="text-[#1DBAEA] text-sm hover:underline">
                      {t('employee.viewAll')}
                    </button>
                  </div>
                        ...
                        <div key={n.id} className={`p-3 rounded-xl ${n.is_read ? 'bg-gray-50' : 'bg-blue-50'}`}>
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <p className="text-sm font-medium text-[#2B3E8F]">{n.message}</p>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <div className="flex items-center gap-2 mb-4">
                    <Activity className="w-5 h-5 text-[#1DBAEA]" />
                    <h2 className="text-lg font-bold text-[#2B3E8F]">{t('employee.summary.activitySummary')}</h2>
                  </div>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={i} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
                        ...
                        <span className="font-bold text-[#2B3E8F]">{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
